...
export function signupValidator(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!validator.isMobilePhone(phone)) throw new Error("Invalid phone number format");
      ...
      if (!validSpecializations.includes(specialization)) throw new Error("Invalid specialization selected");
  
      const exp = Number(experience);
      if (isNaN(exp) || exp < 1 || exp > 50) throw new Error("Experience must be between 1 and 50 years");
